...
class PacketFlag:   # PacketType.PING
    ...
    def get(val : int) -> str:   # PacketGlag.get(1|2)
        flags = ""
        if val == 0:
           return "NONE "
...
class Okon:
    def __init__(self, okon_client) -> None:
        ...
        self.control = {
            ...
            'manual': {
                "FLH":0, "FLV":0, "BLV":0, "BLH":0, "FRH":0, "FRV":0, "BRV":0, "BRH":0
            }
        }
        self.orien = {
            'pos' : { 'x' : 0, 'y' : 0, 'z' : 0},
    ...
    def reachedTargetRotation(self, delta):
        return angle_difference(self.control['stable']['rot']['x'], self.sens['imu']['rot']['x']) < delta and \
            angle_difference(self.control['stable']['rot']['y'], self.sens['imu']['rot']['y']) < delta and \
            angle_difference(self.control['stable']['rot']['z'], self.sens['imu']['rot']['z']) < delta
    ...
    def get_detection(self, className : str):
        return list(filter(lambda d: d['className'] == className and d['visibleInFrame'], self.sens['detection']))
...
class OkonClient:
    def __init__(self, ip, port, options=None, sync_interval = .05, debug = True) -> None:
        self.okon = Okon(self)
    ...
    def _handle_packet(self, packet_type : int, packet_flag : int, data_bytes : bytes):
        if self.debug and packet_flag & PacketFlag.DO_NOT_LOG_PACKET == 0:
            print(f'RECV {PacketType.get(packet_type)} {PacketFlag.get(packet_flag)} len:{len(data_bytes)} {data_bytes.decode("utf-8")[:40]}')
