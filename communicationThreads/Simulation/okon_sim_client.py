...
class Simulation:
    ...
    def sync(self) -> None:
        ...
        self._okon_client.send(PacketType.GET_VIDEO_BYTES, PacketFlag.DO_NOT_LOG_PACKET)
        self._okon_client.send(PacketType.GET_DEPTH_BYTES, PacketFlag.DO_NOT_LOG_PACKET)
...
class OkonSimClient:
    ...
    def _handle_packet(self, packet_type: int, packet_flag: int, data_bytes: bytes):
        ...
        if packet_type == PacketType.SET_MTR:
            ...
            self.okon.depth = data_bytes
        elif packet_type == PacketType.GET_VIDEO_BYTES:
            self.okon.video = data_bytes
